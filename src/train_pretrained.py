...
class DatasetCifar10(torch.utils.data.Dataset):
    def __init__(self, meta, img_size:Tuple[int,int]):
...
ds_tr = DatasetCifar10(meta=meta_tr,img_size=(256,256))
dl_tr = torch.utils.data.DataLoader(dataset = ds_tr,batch_size=32,shuffle=True)
ds_vl = DatasetCifar10(meta=meta_vl,img_size=(256,256))
...
class MyModel(nn.Module):
    def __init__(self):
        ...
        self.NHWC2NCHW = NHWC2NCHW
        self.back_bone = torchvision.models.resnet18(pretrained=True)
        self.back_bone.fc = nn.Linear(512,10)
        
    def forward(self, x):        
        z = self.NHWC2NCHW(x)   
        logits = self.back_bone(z)        
        return logits
